...
在 10.4 節我們看到可以透過反射來分析一個結構體。本節我們進一步探討強大的反射功能。反射是用程序檢查其所擁有的結構，尤其是類型的一種能力；這是元編程的一種形式。反射可以在運行時檢查類型和變數，例如它的大小、方法和 `動態`
的調用這些方法。這對於沒有原始碼的包尤其有用。這是一個強大的工具，除非真得有必要，否則應當避免使用或小心使用。

變數的最基本訊息就是類型和值：反射包的 `Type` 用來表示一個 Go 類型，反射包的 `Value` 為 Go 值提供了反射介面。

兩個簡單的函數，`reflect.TypeOf` 和 `reflect.ValueOf`，返回被檢查對象的類型和值。例如，x 被定義為：`var x float64 = 3.4`，那麼 `reflect.TypeOf(x)` 返回 `float64`，`reflect.ValueOf(x)` 返回 `<float64 Value>`

實際上，反射是通過檢查一個介面的值，變數首先被轉換成空介面。這從下面兩個函數簽名能夠很明顯的看出來：
...
對於 float64 類型的變數 x，如果 `v:=reflect.ValueOf(x)`，那麼 `v.Kind()` 返回 `reflect.Float64` ，所以下面的表達式是 `true`
...
為什麼會這樣呢？問題的原因是 v 不是可設置的（這裡並不是說值不可定址）。是否可設置是 Value 的一個屬性，並且不是所有的反射值都有這個屬性：可以使用 `CanSet()` 方法測試是否可設置。
...
有些時候需要反射一個結構類型。`NumField()` 方法返回結構內的欄位數量；通過一個 for 循環用索引取得每個欄位的值 `Field(i)`。
...
這是因為結構中只有被導出欄位（首字母大寫）才是可設置的；來看下面的例子：
