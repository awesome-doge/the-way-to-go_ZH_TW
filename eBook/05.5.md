...
因此每次疊代都會對條件進行檢查（i < 0），以此判斷是否需要停止循環。如果退出條件滿足，則使用 break 語句退出循環。

一個 break 的作用範圍為該語句出現後的最內部的結構，它可以被用於任何形式的 for 循環（計數器、條件判斷等）。但在 switch 或 select 語句中（詳見第 13 章），break 語句的作用結果是跳過整個代碼塊，執行後續的代碼。

下面的範例中包含了嵌套的循環體（for4.go），break 只會退出最內層的循環：
