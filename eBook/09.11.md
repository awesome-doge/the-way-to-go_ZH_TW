...
當開始一個新項目或增加新的功能到現有的項目，你可以透過在應用程式中使用已經存在的庫來節省開發時間。為了做到這一點，你必須理解庫的 API（應用編程介面），那就是：庫中有哪些方法可以調用，如何調用。你可能沒有這個庫的原始碼，但作者肯定有記載的 API 以及詳細介紹了如何使用它。

作為一個例子，我們將使用Google的 API 的 urlshortener 編寫一個小程序：你可以嘗試一下在 http://goo.gl/ 輸入一個像 "http://www.destandaard.be" 這樣的URL，你會看到一個像 "http://goo.gl/O9SUO" 這樣更短的 URL 返回，也就是說，在 Twitter 之類的服務中這是非常容易嵌入的。Google urlshortener 服務的文件可以在 "http://code.google.com/apis/urlshortener/" 找到。(第 19 章，我們將開發自己版本的 urlshortener)。

Google將這項技術提供給其他開發者，作為 API 我們可以在我們自己的應用程式中調用（釋放到指定的限制）。他們也生成了一個 Go 語言用戶端庫使其變得更容易。

備註：Google讓透過使用 Google API Go 用戶端服務的開發者生活變得更簡單，Go 用戶端程序自動生成於 Google 庫的 JSON 描述。更多詳情在 [項目頁面](http://code.google.com/p/google-api-go-client/) 查看。

下載並安裝 Go 用戶端庫:
將通過 go install 實現。但是首先要驗證環境變數中是否含有 `GOPATH` 變數，因為外部原始碼將被下載到 `$GOPATH/src` 目錄下並被安裝到 `$GOPATH/PKG/"machine_arch"/` 目錄下。

我們將透過在終端調用以下命令來安裝 API:
...
現在我們可以透過導入並賦予別名來使用已安裝的包：

	import  "google.golang.org/api/urlshortener/v1"

現在我們寫一個 Web 應用(參見第 15 章 4-8 節)通過表單實現短地址和長地址的相互轉換。我們將使用 `template` 包並寫三個處理函數：root 函數透過執行表單模板來展示表單。short 函數將長地址轉換為短地址，long 函數逆向轉換。

要調用 urlshortener 介面必須先通過 http 包中的默認用戶端創建一個服務實例 urlshortenerSvc：  
```go
urlshortenerSvc, _ := urlshortener.New(http.DefaultClient)
```

我們透過調用服務中的 `Url.Insert` 中的 `Do` 方法傳入包含長地址的 `Url` 數據結構從而獲取短地址：
...
我們透過調用服務中的 `Url.Get` 中的 `Do` 方法傳入包含短地址的Url數據結構從而獲取長地址：
...
為了代碼的簡潔我們並沒有檢測返回的錯誤狀態，但是在真實的生產環境的應用中一定要做檢測。
