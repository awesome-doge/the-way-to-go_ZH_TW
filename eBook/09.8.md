# 9.8 自訂包的目錄結構、go install 和 go test

為了示範，我們創建了一個名為 uc 的簡單包，它含有一個 `UpperCase` 函數將字串的所有字母轉換為大寫。當然這並不值得創建一個自訂包，同樣的功能已被包含在 `strings` 包裡，但是同樣的技巧也可以應用在更複雜的包中。

## 9.8.1 自訂包的目錄結構

下面的結構給了你一個好的示範(uc 代表通用包名, 名字為粗體的代表目錄，斜體代表可執行文件):
...
將你的項目放在 goprograms 目錄下(你可以創建一個環境變數 GOPATH，詳見第 2.2/3 章節：在 .profile 和 .bashrc 文件中添加 `export GOPATH=/home/user/goprograms`)，而你的項目將作為 src 的子目錄。uc 包中的功能在 uc.go 中實現。
...
創建一個 uc.a 的測試文件在目錄下，輸出為 PASS 時測試通過。

在第 13.8 節我們將給出另外一個測試例子並進行深入研究。

備註：有可能你當前的用戶不具有足夠的資格使用 go　install(沒有權限)。這種情況下，選擇 root 用戶 su。確保 Go 環境變數和 Go 原始碼路徑也設置給 su，同樣也適用你的普通用戶(詳見第 2.3 節)。

接下來我們創建主程式 ucmain.go:
...
另外複製 uc.a 到 /home/user/goprograms 目錄並創建一個 Makefile 並寫入文本：
...
執行 gomake 編譯 `ucmain.go` 生成可執行文件ucmain

運行 `./ucmain` 顯示: `USING PACKAGE UC!`。

## 9.8.2 本地安裝包

本地包在用戶目錄下，使用給出的目錄結構，以下命令用來從原始碼安裝本地包：
...
你有一個很好的理由去寫平台特定的代碼，例如匯編語言。這種情況下，按照下面的約定是合理的：
...
prog1.go 定義了不同操作系統通用的介面，並將系統特定的代碼寫到 prog1_os.go 中。
