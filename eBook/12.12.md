...
透過網路傳輸的數據必須加密，以防止被 hacker（駭客）讀取或篡改，並且保證發出的數據和收到的數據檢驗和一致。
...
透過調用 `sha1.New()` 創建了一個新的 `hash.Hash` 對象，用來計算 SHA1 校驗值。`Hash` 類型實際上是一個介面，它實現了 `io.Writer` 介面：
...
通過 io.WriteString 或 hasher.Write 將給定的 []byte 附加到當前的 `hash.Hash` 對象中。

**練習 12.9**：[hash_md5.go](exercises/chapter_12/hash_md5.go)：

在範例 12.20 中檢驗 md5 算法。
