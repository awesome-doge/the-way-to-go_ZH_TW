# 11.3 類型斷言：如何檢測和轉換介面變數的類型

一個介面類型的變數 `varI` 中可以包含任何類型的值，必須有一種方式來檢測它的 **動態** 類型，即運行時在變數中存儲的值的實際類型。在執行過程中動態類型可能會有所不同，但是它總是可以分配給介面變數本身的類型。通常我們可以使用 **類型斷言** 來測試在某個時刻 `varI` 是否包含類型 `T` 的值：
...
程序中定義了一個新類型 `Circle`，它也實現了 `Shaper` 介面。 `if t, ok := areaIntf.(*Square); ok ` 測試 `areaIntf` 裡是否有一個包含 `*Square` 類型的變數，結果是確定的；然後我們測試它是否包含一個 `*Circle` 類型的變數，結果是否定的。
